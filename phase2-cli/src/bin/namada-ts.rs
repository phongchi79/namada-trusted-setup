...
fn compute_contribution_offline() -> Result<()> {
    ...
    msg.push_str(
        format!(
            "{}",
            format!(
                "{:4}2) Execute the command \"cargo run --release --bin phase2 --features=cli contribute offline\"\n",
                ""
            )
            .as_str()
            .bright_cyan()
